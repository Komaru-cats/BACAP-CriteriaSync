...
def load_parser():
    ...
    bacapse_super = AdvType(name="super_challenge",frames="challenge",colors=Color("#FF2A2A"))

    manager = AdvTypeManager(task, goal, challenge, super_challenge, root, milestone, advancement_legend)
    task_terralith = AdvType(name="task", frames="task", colors=Color("green"), hidden_color=constants.DEFAULT_BACAP_HIDDEN_COLOR)
    ...
    cereal_dedication_manager = AdvTypeManager(task_terralith, goal, challenge, super_challenge, root, milestone, advancement_legend)
    se_manager = AdvTypeManager(task, goal, challenge, bacapse_super, root, milestone, advancement_legend)
    ...
    cereal_dedication = Datapack(name="cereal_dedication", path=Path("datapacks/cereal_dedication"), adv_type_manager=cereal_dedication_manager, reward_namespace="bacap_rewards", technical_tabs="technical")
    ...
    bacapse = Datapack(name="bacapse", path=Path("datapacks/bacapse"), adv_type_manager=se_manager, reward_namespace="suggestion_rewards", technical_tabs="technical")

    return Parser(bacap, bacaped, bacaped_hardcore, bacap_hardcore, incendium, bacap_terralith,
                  bacap_nullscapes, bacap_amplified_nether, cereal_dedication_hardcore, cereal_dedication, complete_collection, bacapse)
